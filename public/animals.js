...
setInterval(() => {
  ...
  for (const el of root.querySelectorAll('[data-animal-status]')) {
    ...
    if(pet && el.dataset.cycle!==`${pet.readyAt}:${Math.max(0,Math.min(100,Number(pet.health ?? 100)))}:${pet.booster?'booster':''}`)changed=true;
