...
(function(root){
  ...
  function pulse(state,{now=Date.now(),random=Math.random,maxWater,ready,accessible=()=>true}={}){
    if(state.selectedMascot!=='premium'||state.secondaryMascot!=='rainbow'||state.ownedMascots?.rainbow!==true)return {changed:false,harvest:-1,wateredPlot:-1,wateredAll:false};
