...
function renderAnimalYard() {
  ...
  root.innerHTML = `<div class="animal-intro"><div><h3>Um cantinho de carinho</h3><p>Alimente, espere e recolha. Cada bichinho tem seu próprio lar.</p></div></div><div class="animal-pens">${Object.entries(FarmAnimals.catalog).map(([id,a]) => {
    ...
    const rationButton=(r,label,qty)=>`<button type="button" class="animal-feed-choice animal-feed-tooltip ${qty?'':'empty'}" data-tooltip="Adicionar ${label} — ${FarmAnimals.rations[r].description}" aria-label="Adicionar ${label}" onclick="feedAnimalChoice('${id}','${r}')" ${qty||animalActionInFlight?'':'disabled'}>${rationArt(r)}<small>${qty}</small></button>`;
    const choice=pet?`<div class="animal-feed-picker" aria-label="Escolha uma ração">${rationButton('normal','Ração Normal',state.feed)}${rationButton('premium','Ração Premium',state.rations.premium)}${rationButton('super','Ração Super Premium',state.rations.super)}</div>`:'';
    ...
    return `<article class="animal-pen ${status}">${collect}<span class="animal-home">${a.home}</span><h4>${petName}</h4><div class="animal-scene ${status}">${animalArt(id)}</div>${pet?`<input class="animal-name-input" maxlength="15" value="${petName.replace(/"/g,'&quot;')}" aria-label="Nome do animal" onchange="animalAction('rename','${id}',this.value)" />`:''}<div class="animal-status" data-animal-status="${id}" data-cycle="${pet?`${pet.readyAt}:${stocked}`:''}">${message}</div>${reserve}${choice}<div class="animal-progress"><span style="width:${Math.max(0,progress)}%"></span></div>${status==='empty'?`<button class="animal-action" onclick="${action}">Conhecer na loja</button>`:''}${booster}</article>`;
...
function renderAnimalShop() {
  ...
  const state = animalState();
  root.innerHTML = `<p class="animal-note">Cada compra inclui o animal e seu cercadinho. Um de cada espécie por fazenda. Compre comida na aba Rações. Os produtos recolhidos ficam no Mercado.</p><div class="animal-shop-grid">${Object.entries(FarmAnimals.catalog).map(([id,a]) => {
...
setInterval(() => {
  ...
  for (const el of root.querySelectorAll('[data-animal-status]')) {
    ...
    if (FarmAnimals.status(pet) === 'producing') {
      ...
      el.innerHTML = `${FarmAnimals.products[a.product].name} em ${animalTime(remaining)} <span class="animal-ration-tag ${pet.ration||'normal'}">${FarmAnimals.rations[pet.ration||'normal'].name}</span>`;
