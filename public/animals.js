...
function renderAnimalYard() {
  ...
  root.innerHTML = `<div class="animal-intro"><div><h3>Um cantinho de carinho</h3><p>Alimente, espere e recolha. Cada bichinho tem seu próprio lar.</p></div></div><div class="animal-pens">${Object.entries(FarmAnimals.catalog).map(([id,a]) => {
    ...
    const superRemaining=pet?.superActive?Math.max(0,pet.superUntil-now):0;
    ...
    const superStatus=pet?`<div class="animal-super-status ${pet.superActive?'active':''}"><div class="animal-booster-heading"><span>✦ Bônus Super Premium</span><b class="animal-super-time">${pet.superActive?`ativo · ${animalTime(superRemaining)}`:'inativo'}</b></div><div class="animal-booster-bar animal-super-bar"><span style="width:${pet.superActive?Math.max(0,Math.min(100,superRemaining/(30*60000)*100)):0}%"></span></div></div>`:'';
  ...
  }).join('')}</div><p class="animal-note">Com 15% ou mais de saúde, o animal produz normalmente. Cada ração recupera saúde e os produtos entram automaticamente no Mercado. Super Premium: recupera 80% da saúde e concede por 30 minutos 34% de chance de produzir 1 item extra. Booster: 30 minutos de efeito.</p><button class="animal-action" onclick="openRationShop()">Comprar rações e Booster</button>`;
}
...
setInterval(() => {
  ...
  for (const el of root.querySelectorAll('[data-animal-status]')) {
    ...
    if(superTime&&superBar){
      const remaining=pet?.superActive?Math.max(0,pet.superUntil-Date.now()):0;
      superTime.textContent=pet?.superActive?`ativo · ${animalTime(remaining)}`:'inativo';
