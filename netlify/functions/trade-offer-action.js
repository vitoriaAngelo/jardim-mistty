...
exports.handler=async event=>{
  ...
  try{
    ...
    const response=await fetch(SUPABASE_URL+'/rest/v1/rpc/'+rpcName,{method:'POST',headers:{apikey:KEY,Authorization:'Bearer '+KEY,'Content-Type':'application/json'},body:JSON.stringify(args)});
    ...
    let albumCards=null;
    if(action==='accept'){
      const garden=await fetch(SUPABASE_URL+'/rest/v1/gardens?username=eq.'+encodeURIComponent(user.username)+'&select=data,updated_at&order=updated_at.desc&limit=1',{headers:{apikey:KEY,Authorization:'Bearer '+KEY}});
