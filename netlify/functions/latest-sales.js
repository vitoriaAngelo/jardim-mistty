...
const activeDbKey = () => process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY || SUPABASE_KEY;
...
exports.handler = async () => {
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/gardens?select=username,data&order=updated_at.desc&limit=20`, { headers:{ apikey:activeDbKey(), Authorization:`Bearer ${activeDbKey()}` } });
